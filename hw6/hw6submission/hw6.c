...
int visitAddress(Set *set, int tag)
{
    ...
    for (int x = 0; x <= set->numRows; x++) {
        ...
        // if the tag found and valid
        if (row->tag == tag && row->valid) {
            return HIT;
        }
        
        // if tag not found but there is an empty slot
        if (!row->valid) {
            row->valid = 1;
            row->tag = tag;
            return MISS;
        ...
        // if the tag not found and set is full, evict lowest version
        if (x == set->numRows - 1) {
...
void visitAddresses(Cache *cache, int len, int addresses[])
{
    if (cache->mode == 0) 
    {
    ...
    for (int x = 0; x < len; x++) {
        ...
        // get address
        int address = addresses[x];

        // compute tag, index, offset
        int offset, index, tag;
        tag = address / (cache->s / cache->n);
        ...
        {
            index = address / (cache->l);
        }
        
        offset = address % cache->l;
        
        // get set
        Set *set = &cache->sets[index];
        
        // test for hit
        int hit = visitAddress(set, tag);
        
        // log to console
        printf("%i\t: %s\t- %d %d %d\n", addresses[x], hit ? "HIT" : "MISS", tag, index, offset);
    }
