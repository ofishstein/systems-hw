...
static void execute(int argc, char *argv[])
{
    ...
    pid_t childpid = -2; /* child process ID */
    pid_t childpid2 = -2;
    ...
    if (childpid == -1 || childpid2 == -1) { /* in parent (returned error) */
	 ...
	 *  the directories specified by the environment variable PATH.
        ...
        if(fout != 0 ) {
            ...
            if(open(argv[cIndex + 1], O_WRONLY | O_CREAT, S_IRUSR | S_IWUSR) == -1) {
                perror("opening write");
            }
        }
        ...
        if(fin != 0) {
            ...
            if(open(argv[cIndex + 1], O_RDONLY) == -1) {
                perror("opening read");
            }
        }
	...
	/* NOT REACHED unless error occurred */
        ...
        *argv = argv[cIndex + 1];
        ...
        } else if (bg == 0 && childpid2 > 0) {
            waitpid(childpid2, NULL, 0); /* wait until child process finishes */
        } else {
            printf("[%d] sent to bg\n", childpid2);
        }
